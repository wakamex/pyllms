...
class LLMS:
    ...
    def __init__(self, model: str, **kwargs):
        ...
        googs = GoogleProvider(model="chat-bison", **kwargs)
        print(f"{googs=}")

        for single_model in self._models:
            print(f"looking up {single_model}")
            for provider in self._possible_providers:
                if single_model in provider.provider.MODEL_INFO:
                    ...
                    if provider.api_key:
                        print(f"instantiating with api key {provider.api_key}")
                        ...
                        print("instantiating without api key")
